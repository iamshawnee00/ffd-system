...
export default function ProductManagementPage() {
  ...
  const handleExportCSV = async () => {
    ...
    try {
        ...
        if (convData) {
            convData.forEach(c => {
                if (!convMap[c.ProductCode]) convMap[c.ProductCode] = [];
                convMap[c.ProductCode].push(`1 ${c.ConversionUOM} = ${c.Factor} ${c.BaseUOM}`);
            });
        }
        ...
        filteredAndSortedProducts.forEach(p => {
            ...
            const row = [
                ...
                escapeCSV(p.ChineseName || ''),
                escapeCSV(p.Category),
                escapeCSV(p.origin),
                escapeCSV(p.BaseUOM),
                escapeCSV(p.SalesUOM),
                escapeCSV(p.PurchaseUOM),
                escapeCSV(p.AllowedUOMs),
                escapeCSV(conversionsStr),
        ...
        link.setAttribute("download", `Product_Masterlist_${new Date().toISOString().split('T')[0]}.csv`);
