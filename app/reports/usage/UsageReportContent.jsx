...
export default function UsageReportContent() {
  ...
  const formatDate = (dStr) => {
      ...
      return !isNaN(d) ? d.toLocaleDateString('en-CA') : dStr;
  };
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-4 md:p-8 print:p-0 print:bg-white text-black font-sans text-xs">
      ...
      <button 
        ...
        className="fixed bottom-4 right-4 md:bottom-8 md:right-8 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-full shadow-2xl print:hidden flex items-center gap-2 z-50 transition-all transform hover:scale-105"
      >
        <span className="text-xl">🖨️</span> <span className="hidden md:inline">Print Report</span>
      </button>

      {loading && <div className="p-10 text-center">Generating Usage Report...</div>}
      {!loading && items.length === 0 && <div className="p-10 text-center text-gray-500">No orders found for {date}.</div>}

      {/* MOBILE VIEW (Visible on small screens, Hidden on print) */}
      {!loading && items.length > 0 && (
        <div className="block md:hidden print:hidden space-y-4 mb-20">
          <div className="bg-white p-4 rounded-lg shadow mb-4">
            <h2 className="text-xl font-bold uppercase mb-1">Daily Usage</h2>
      ...
      {/* DESKTOP/PRINT VIEW (Hidden on mobile, Visible on desktop & print) */}
