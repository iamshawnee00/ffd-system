      ...
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full sm:w-auto">
             <button 
                ...
                disabled={isSyncing}
                className="flex-1 sm:flex-none bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-black py-3 px-5 rounded-2xl text-[10px] md:text-xs uppercase tracking-widest border border-indigo-200 shadow-sm transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50"
             >
             ...
             <button 
                ...
                className="flex-1 sm:flex-none bg-purple-600 hover:bg-purple-700 text-white font-black py-3 px-5 rounded-2xl text-[10px] md:text-xs uppercase tracking-widest shadow-sm transition-all active:scale-95 flex items-center justify-center gap-2"
             >
                <span>📦</span> <span className="hidden sm:inline">All DOs</span>
             </button>
             <button 
                onClick={() => window.open(`/reports/usage?date=${selectedDate}`, '_blank')} 
                className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white font-black py-3 px-5 rounded-2xl text-[10px] md:text-xs uppercase tracking-widest shadow-sm transition-all active:scale-95 flex items-center justify-center gap-2"
             >
                ...
                return (
                  ...
                  >
                      ...
                      {count > 0 ? (
                          ...
                          <div className="w-full text-center py-1 sm:py-1.5 rounded-lg text-[8px] sm:text-[9px] font-bold text-gray-400 bg-gray-50 mt-2 sm:mt-auto">
                              None
                          </div>
                      )}
                  </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-xl border border-gray-100 overflow-hidden flex flex-col">
          ...
          <div className="p-5 md:p-6 border-b border-gray-100 bg-white relative">
              <div className="relative w-full">
                  <span className="absolute left-5 top-4 text-gray-400"><MagnifyingGlassIcon className="w-5 h-5"/></span>
                  <input 
                      type="text" 
          ...
          <div className="border-b border-gray-100">
              <button 
                  ...
                  className="w-full flex justify-between items-center px-5 md:px-6 py-4 text-purple-700 font-bold bg-purple-50 hover:bg-purple-100 transition-colors text-xs md:text-sm uppercase tracking-widest"
              >
                  <span className="flex items-center gap-2">📋 View Daily Production Usage Summary</span>
                  <span>{isUsageExpanded ? '▲' : '▼'}</span>
              </button>
              {isUsageExpanded && (
                  <div className="max-h-60 overflow-y-auto p-5 md:p-6 bg-purple-50/30 border-t border-purple-100 custom-scrollbar">
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-3 text-xs md:text-sm">
                          {usageSummary.map((u, i) => (
                              <div key={i} className="flex justify-between border-b border-purple-100 pb-2">
                                  <span className="text-gray-700 font-bold truncate pr-2 uppercase">{u.name}</span>
                                  <span className="font-black text-purple-800 whitespace-nowrap">{u.qty} <span className="text-[10px] font-bold text-gray-400 uppercase">{u.uom}</span></span>
                              </div>
                          ))}
                      </div>
                  </div>
              ...
              <table className="w-full text-left min-w-[1050px]">
                  <thead className="bg-gray-50 text-[10px] font-black text-gray-400 uppercase tracking-widest sticky top-0 z-10 shadow-sm border-b border-gray-100">
                      <tr>
                          ...
                          <th className="p-5 w-[350px]">Customer Info</th>
                          <th className="p-5 w-[220px]">Logistics</th>
                          <th className="p-5 text-center w-16">Items</th>
                          <th className="p-5 w-28">Driver</th>
                          <th className="p-5 text-center w-28">Live Status</th>
                          <th className="p-5 text-right w-24 pr-6">Action</th>
                      </tr>
                  </thead>
                      ...
                      {filteredGroupedOrders.length === 0 ? (
                          ...
                          filteredGroupedOrders.map(group => {
                              ...
                              >
                                  ...
                                  <td className="p-4">
                                      <span className="font-mono text-xs font-black text-green-700 bg-green-100 px-2 py-1 rounded border border-green-200">
                                          {group.info.DONumber}
                                      </span>
                                  </td>
                                  <td className="p-4">
                                      ...
                                      <div className="text-xs text-gray-500 mt-1 font-bold">{group.info["Contact Person"]}</div>
                                  </td>
                                  <td className="p-4">
                                      <div className={`text-[9px] px-2 py-1 rounded w-fit font-black mb-1 uppercase tracking-widest border ${getDeliveryModeStyle(group.info["Delivery Mode"])}`}>
                                  ...
                                  <td className="p-4">
                                      {group.info.DriverName ? (
                                          ...
                                          <span className="text-gray-300 text-[10px] font-bold uppercase tracking-widest italic">Unassigned</span>
                                      )}
                                  </td>
                                  <td className="p-4 text-center">
                                  ...
                                  <td className="p-4 text-right pr-6" onClick={(e) => e.stopPropagation()}>
                                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover/row:opacity-100 transition-opacity w-full">
                                          ...
                                          <button onClick={() => handleSendToShipday(group)} className="p-1.5 text-green-600 hover:bg-green-100 rounded-lg transition" title="Push to Shipday"><TruckIcon className="w-5 h-5" /></button>
          ...
          <div className="md:hidden flex flex-col gap-3 p-3 bg-gray-50/50">
              ...
              {filteredGroupedOrders.length > 0 && (
                  <div className="flex items-center gap-3 px-3 py-2 bg-white rounded-xl shadow-sm border border-gray-100">
                      <input 
                          type="checkbox" 
              ...
              {filteredGroupedOrders.length === 0 ? (
                  <div className="p-10 text-center text-gray-400 italic font-bold bg-white rounded-2xl border border-dashed border-gray-200">No orders found.</div>
              ) : (
                  filteredGroupedOrders.map(group => {
                      const rawStatus = getRawStatus(group.info);
                      const isSelected = selectedDOs.has(group.info.DONumber);
                          ...
                          >
                              ...
                              <div className="flex justify-between items-start mb-3">
                                  <div className="flex flex-col gap-1.5">
                                      <span className="font-mono text-[10px] font-black text-green-700 bg-green-100 px-2 py-1 rounded border border-green-200 w-fit">
                                          {group.info.DONumber}
                                      </span>
                              ...
                              <div className="flex flex-wrap items-center gap-2 mb-4 border-t border-gray-50 pt-3">
                                  ...
                                  {group.info.DriverName ? (
                                       <span className="bg-indigo-50 text-indigo-700 text-[9px] font-black px-2 py-1 rounded-md border border-indigo-100 uppercase truncate max-w-[100px]">{group.info.DriverName}</span>
                                  ) : (
      ...
      {selectedDOs.size > 0 && (
          <div className="fixed bottom-4 md:bottom-8 left-1/2 transform -translate-x-1/2 w-[92%] sm:w-max bg-gray-900/95 backdrop-blur-xl text-white p-3 sm:px-6 sm:py-4 rounded-2xl sm:rounded-full shadow-[0_20px_40px_rgba(0,0,0,0.4)] flex flex-col sm:flex-row items-center gap-3 sm:gap-6 z-[100] animate-in slide-in-from-bottom-10 border border-gray-700">
              ...
              <div className="flex items-center justify-between w-full sm:w-auto sm:border-r border-gray-700 sm:pr-6 shrink-0">
                  ...
                  <button onClick={() => setSelectedDOs(new Set())} className="sm:hidden text-gray-400 hover:text-white bg-gray-800 p-1.5 rounded-full transition">
              ...
              <div className="flex gap-2 w-full sm:w-auto overflow-x-auto custom-scrollbar pb-1 sm:pb-0 snap-x">
                  ...
                  <button onClick={sendSelectedToShipday} className="flex items-center gap-2 bg-gray-800 sm:bg-transparent hover:bg-white/10 px-4 py-2.5 sm:py-2 rounded-xl transition font-bold text-[10px] md:text-xs shrink-0 snap-start border border-gray-700 sm:border-none">
              ...
              <button onClick={() => setSelectedDOs(new Set())} className="hidden sm:block text-gray-400 hover:text-white transition bg-gray-800 p-2 rounded-full hover:bg-gray-700 shrink-0" title="Clear Selection">
                  <XMarkIcon className="w-5 h-5" />
              </button>
          </div>
      )}
          ...
          <div className="fixed inset-0 bg-black/60 z-[110] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-[2.5rem] w-full max-w-5xl p-8 shadow-2xl flex flex-col max-h-[95vh] animate-in zoom-in duration-200 border border-gray-100">
                <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-6 shrink-0">
                    <div>
                        <h2 className="text-xl md:text-2xl font-black text-gray-800 uppercase flex items-center gap-2">
                            Edit Master Order 
                            <span className="text-blue-600 font-mono tracking-tighter">{editingOrder.DONumber}</span>
                        </h2>
                        <div className="flex items-center gap-3 mt-3">
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Manual Status Override:</span>
                            <select 
                                className={`border rounded-xl px-4 py-1.5 text-[10px] font-black outline-none shadow-sm transition-all ${getStatusColor(getRawStatus(editingOrder))}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 shrink-0 bg-gray-50/50 p-6 rounded-3xl border border-gray-100 text-xs font-bold uppercase shadow-inner">
                    <div className="md:col-span-2">
                        <label className="block text-[9px] text-gray-400 mb-1.5 ml-1">Customer Name</label>
                        <input className="w-full p-3 border border-gray-200 rounded-2xl outline-none font-black focus:ring-2 focus:ring-blue-500" value={editingOrder["Customer Name"]} onChange={e => setEditingOrder({...editingOrder, "Customer Name": e.target.value})} />
                    </div>
                    <div className="md:col-span-2">
                        <label className="block text-[9px] text-gray-400 mb-1.5 ml-1">Delivery Address</label>
                        <input className="w-full p-3 border border-gray-200 rounded-2xl outline-none font-medium focus:ring-2 focus:ring-blue-500" value={editingOrder["Delivery Address"]} onChange={e => setEditingOrder({...editingOrder, "Delivery Address": e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-[9px] text-gray-400 mb-1.5 ml-1">Contact Person</label>
                        <input className="w-full p-3 border border-gray-200 rounded-2xl outline-none font-black focus:ring-2 focus:ring-blue-500" value={editingOrder["Contact Person"] || ''} onChange={e => setEditingOrder({...editingOrder, "Contact Person": e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-[9px] text-gray-400 mb-1.5 ml-1">Contact Number</label>
                        <input className="w-full p-3 border border-gray-200 rounded-2xl outline-none font-black focus:ring-2 focus:ring-blue-500" value={editingOrder["Contact Number"] || ''} onChange={e => setEditingOrder({...editingOrder, "Contact Number": e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-[9px] text-gray-400 mb-1.5 ml-1">Delivery Date</label>
                        <input type="date" className="w-full p-3 border border-gray-200 rounded-2xl outline-none font-black bg-blue-50 text-blue-800 focus:ring-2 focus:ring-blue-500" value={editingOrder["Delivery Date"]} onChange={e => setEditingOrder({...editingOrder, "Delivery Date": e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-[9px] text-gray-400 mb-1.5 ml-1">Mode</label>
                        <select className="w-full p-3 border border-gray-200 rounded-2xl outline-none font-black focus:ring-2 focus:ring-blue-500" value={editingOrder["Delivery Mode"] || 'Driver'} onChange={e => setEditingOrder({...editingOrder, "Delivery Mode": e.target.value})}>
                            <option value="Driver">Driver</option>
                            <option value="Lalamove">Lalamove</option>
                            <option value="Self Pick-up">Self Pick-up</option>
                        </select>
                ...
                <div className="flex-1 overflow-auto border border-gray-100 rounded-3xl mb-6 custom-scrollbar bg-white shadow-inner">
                    <table className="w-full text-left text-xs whitespace-nowrap">
                        <thead className="bg-gray-100/50 font-black text-gray-500 sticky top-0 z-10 text-[10px] uppercase tracking-widest border-b border-gray-100">
                            <tr>
                                <th className="p-4 pl-6">Product Catalog Item</th>
                                ...
                                <th className="p-4 w-24 text-center">REP?</th>
                                <th className="p-4 w-12 text-center pr-6"></th>
                            </tr>
                        </thead>
                            ...
                            {editingItems.map((item, idx) => (
                                <tr key={idx} className={item.Replacement === 'YES' ? 'bg-red-50/20' : 'hover:bg-gray-50/30'}>
                                    ...
                                    <td className="p-3 text-center">
                                        <input type="number" className="w-full p-2.5 border border-gray-200 rounded-xl text-center font-black outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" value={item.Quantity} onChange={e => handleEditItemChange(idx, 'Quantity', e.target.value)} />
                                    </td>
                                    <td className="p-3 text-center">
                                        <select className="w-full p-2.5 border border-gray-200 rounded-xl text-center font-bold uppercase outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" value={item.UOM} onChange={e => handleEditItemChange(idx, 'UOM', e.target.value)}>
                                            {(() => {
                                                ...
                                                return Array.from(new Set([item.UOM, ...uoms])).map(u => <option key={u} value={u}>{u}</option>);
                                            })()}
                                        </select>
                                    </td>
                                    <td className="p-3 text-right">
                                        <input type="number" step="0.01" className="w-full p-2.5 border border-gray-200 rounded-xl text-right font-black outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" value={item.Price} onChange={e => handleEditItemChange(idx, 'Price', e.target.value)} disabled={item.Replacement === 'YES'} />
                                    </td>
                                    <td className="p-3 text-center">
                                        <input type="checkbox" className="w-5 h-5 text-red-500 rounded border-gray-300 focus:ring-red-500 cursor-pointer shadow-sm" checked={item.Replacement === 'YES'} onChange={e => { handleEditItemChange(idx, 'Replacement', e.target.checked ? 'YES' : ''); if (e.target.checked) handleEditItemChange(idx, 'Price', 0); }} />
                                    </td>
                ...
                {/* NOTE: You are editing the final order, to add fully new items, recreate the order for now or modify list directly */}
